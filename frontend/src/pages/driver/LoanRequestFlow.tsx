...
function LoanRequestFlow() {
  ...
  const [canRequestFromDate, setCanRequestFromDate] = useState<string | null>(null);
  ...
  const loadDriverInfo = async () => {
    ...
    try {
      ...
      if (dashboardData?.driver) {
        setDriverInfo(dashboardData.driver);
      }

      // El backend ya retorna si tiene préstamo activo o solicitud pendiente con sus mensajes
      if (dashboardData?.activeLoan?.message) {
        setIsBlocked(true);
        setBlockingMessage(dashboardData.activeLoan.message);
        setCanRequestFromDate(dashboardData.activeLoan.canRequestFromDate || null);
      } else if (dashboardData?.pendingRequest?.message) {
        setIsBlocked(true);
        setBlockingMessage(dashboardData.pendingRequest.message);
        setCanRequestFromDate(null);
      } else {
        setCanRequestFromDate(null);
      }
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 lg:p-8">
          <div className="flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div>
              ...
              {canRequestFromDate && (
                <p className="mt-4 text-base font-semibold text-red-600 bg-red-50 border border-red-200 rounded-lg py-3 px-4 max-w-md mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-6">
              <button
                ...
                className="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-semibold py-3 px-6 rounded-lg transition-all shadow-md flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-3 px-6 rounded-lg transition-all flex items-center justify-center gap-2"
              >
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 py-5 lg:py-6 px-4 lg:px-6">
        <div className="flex items-center justify-center gap-1 lg:gap-2 overflow-x-auto scrollbar-hide">
          {steps.map((step, idx) => {
            ...
            return (
              <div key={step.number} className="flex items-center flex-shrink-0">
                <div className="flex flex-col items-center min-w-[70px] lg:min-w-[90px] pt-1">
                  <div
                    className={`w-10 h-10 lg:w-12 lg:h-12 rounded-lg flex items-center justify-center font-bold text-sm transition-all duration-300 ${
                      isActive
                  ...
                  <span className={`text-[10px] lg:text-xs font-semibold mt-1.5 text-center leading-tight px-1 ${
                    isActive ? 'text-red-600' : isCompleted ? 'text-green-600' : 'text-gray-400'
                  }`}>
                    {step.number === 4 ? (offer?.requiresGuarantor ? 'Garante' : 'Persona de Contacto') : step.name}
                  </span>
                </div>
                {idx < steps.length - 1 && (
                  <div className={`w-4 lg:w-8 h-0.5 mx-0.5 lg:mx-1 transition-all duration-300 flex-shrink-0 ${
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-5 lg:p-6 animate-fade-in">
        {renderStepContent()}
        
        {/* Información Importante */}
        {currentStep === 1 && (
          <div className="bg-[#fefcea] rounded-lg border-l-2 border-[#e08a00] p-3 lg:p-4 mt-6">
      ...
      <div className="flex items-center justify-between gap-4 mt-8">
        <button
          ...
          className="flex items-center gap-2 px-6 py-3.5 bg-white border-2 border-gray-300 text-gray-700 rounded-xl font-bold hover:bg-gray-50 hover:border-gray-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5 text-base"
        >
          <ArrowLeft className="w-5 h-5" />
          Atrás
        </button>
    ...
    return (
      <div className="flex items-center justify-center py-16">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg font-medium">Cargando información del conductor...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Verificación de Identidad</h2>
            <p className="text-xs text-gray-600 mt-0.5">Información verificada y validada</p>
          </div>
      ...
      {/* Información del conductor - Compacta */}
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
        <div className="border-b border-gray-200 px-4 py-3">
          <h3 className="text-base font-semibold text-gray-900">Datos del Conductor</h3>
        </div>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <div>
          ...
          <p className="text-xs text-gray-600 mt-0.5">Selecciona dónde deseas recibir tu préstamo</p>
        </div>
      </div>
        ...
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <label className={`flex items-center gap-3 p-4 border-2 rounded-lg cursor-pointer transition-colors ${
              ...
              />
              <div className="flex-1">
                <div className="font-semibold text-sm text-gray-900 mb-0.5">YANGO PRO</div>
                <span className="text-xs text-gray-600">A mi saldo de Yango Pro. El mismo dia que se aprueba la solicitud, el dinero sera acreditado a mi saldo de Yango Pro.</span>
              </div>
            </label>
            <label className={`flex items-center gap-3 p-4 border-2 rounded-lg cursor-pointer transition-colors ${
              ...
              />
              <div className="flex-1">
                <div className="font-semibold text-sm text-gray-900 mb-0.5">CUENTA BANCARIA</div>
                <span className="text-xs text-gray-600">A una cuenta bancaria. El desembolso se realizará en el plazo de 24 horas hábiles</span>
              </div>
            </label>
          </div>
        </div>
      ...
      {formData.depositType === 'bank' && (
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
          ...
          <div className="border-b border-gray-200 px-4 py-3">
            ...
            <p className="text-xs text-gray-600">Solo hacemos depósitos a estos bancos</p>
          </div>
          <div className="p-4">
              ...
              {['BCP', 'BBVA', 'INTERBANK'].map((bank) => (
                ...
                >
                  <input
                    ...
                    className="w-4 h-4 text-red-600 mb-1.5"
                  />
                  <span className="font-semibold text-xs text-gray-900">{bank}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="border-t border-gray-200 px-4 py-3">
            ...
            <p className="text-xs text-gray-600">No aceptamos cuentas en Dólares para este tipo de depósitos</p>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              {['CUENTA DE AHORRO'].map((type) => (
                ...
                >
                  ...
                  />
                  <span className="font-medium text-xs text-gray-900 text-center leading-tight">{type}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="border-t border-gray-200 px-4 py-4">
            ...
            <input
              ...
              className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-600 focus:outline-none text-sm"
            />
            <p className="text-xs text-gray-600 mt-2 bg-blue-50 p-2.5 rounded-lg border border-blue-200 leading-relaxed">
...
function Step3LoanRequest({ formData, setFormData, offer, country = 'PE' }: any) {
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Solicitud de Préstamo</h2>
          <p className="text-xs text-gray-600 mt-0.5">Indica el monto y propósito de tu préstamo</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <div className="w-2 h-2 bg-green-500 rounded-full"></div>
              <p className="text-xs text-gray-600">Según tu ciclo {offer.cycle}</p>
            </div>
            <p className="text-base font-semibold text-gray-900">
              Puedes solicitar hasta <span className="text-red-600 text-lg">{formatCurrency(offer.maxAmount, country)}</span>
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
        <div className="border-b border-gray-200 px-4 py-3">
          <h3 className="text-sm font-semibold text-gray-900">Monto del Préstamo</h3>
        </div>
        <div className="p-4 space-y-4">
          <div>
            ...
            <div className="relative">
              <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600 font-semibold text-sm">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-600 focus:outline-none text-sm"
              />
            </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-600 focus:outline-none resize-none text-sm"
            />
            <p className="text-xs text-gray-500 mt-1.5">Mínimo 20 caracteres</p>
          </div>
        </div>
      </div>
    </div>
...
function Step4ContactPerson({ formData, setFormData, contactSignatureRef, clearSignature, handleSignatureStart, handleSignatureMove, handleSignatureEnd, user, driverDocumentNumber, driverPhone, requiresGuarantor }: any) {
  ...
  return (
    <div className="space-y-4">
      {/* Header: Garante (si requiresGuarantor) o Persona de Contacto */}
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <div>
      ...
      {requiresGuarantor ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
            <div className="border-b border-gray-200 px-4 py-3">
              <h3 className="text-sm font-semibold text-gray-900">Datos del Garante</h3>
            </div>
            <div className="p-4 space-y-4">
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-3 py-2 pr-10 border-2 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-600 focus:outline-none text-sm ${
                ...
                {contactDniError ? (
                  <p className="text-xs text-red-600 mt-1">El {cfg.docLabel.toLowerCase()} del garante no puede ser el mismo que el tuyo</p>
                ) : (
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-600 focus:outline-none text-sm ${
              ...
              <div>
                ...
                <div className={`flex items-center rounded-lg border-2 overflow-hidden ${contactPhoneError ? 'border-red-500' : contactPhoneDigits.length === requiredPhoneLen ? 'border-green-500' : 'border-gray-300'}`}>
                  <span className="px-3 py-2 bg-gray-50 border-r border-gray-300 text-gray-600 text-sm font-medium">{cfg.phonePrefix}</span>
                  <input
                    type="text"
                    ...
                    className={`flex-1 px-3 py-2 border-0 focus:ring-2 focus:ring-red-500 focus:outline-none text-sm ${contactPhoneError ? 'bg-red-50/30' : contactPhoneDigits.length === requiredPhoneLen ? 'bg-green-50/30' : ''}`}
                  />
                </div>
                {contactPhoneError && (
                  <p className="text-xs text-red-600 mt-1">El teléfono del garante no puede ser el mismo que el tuyo</p>
                )}
              </div>
              <div>
                ...
                <p className="text-xs text-gray-600 mb-2">Solo cámara. Toma una foto de la parte frontal del {cfg.docPhotoHint} del garante usando el marco como guía.</p>
                {formData.contactFrontPhoto ? (
                  <div className="space-y-2">
                    <div className="border border-gray-300 rounded-lg p-3 bg-gray-50">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-xs font-medium text-gray-900 truncate flex-1">{formData.contactFrontPhoto.name}</p>
                        <button
                          type="button"
                  ...
                  <div>
                    ...
                    {allowFileUpload ? (
                      <button
                        ...
                        className="w-auto min-w-[140px] px-4 py-2.5 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 transition-colors"
                      >
                        Adjuntar foto
                      </button>
                    ) : (
          ...
          {showGaranteCamera && (
            <div className="fixed inset-0 z-50 flex flex-col bg-black">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 flex gap-3 justify-center bg-gradient-to-t from-black/80 to-transparent">
                <button
                  type="button"
                  onClick={closeGaranteCamera}
                  className="flex items-center gap-2 px-5 py-2.5 bg-gray-600 text-white rounded-lg text-sm font-medium hover:bg-gray-500 pointer-events-auto"
                >
                  <X className="w-4 h-4" />
                  Cancelar
                </button>
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 pointer-events-auto"
                >
          ...
          <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
            <div className="border-b border-gray-200 px-4 py-3">
              <h3 className="text-sm font-semibold text-gray-900">Firma del Garante</h3>
            </div>
            <div className="p-4 space-y-3">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <p className="text-xs text-blue-900 font-semibold mb-1">Al firmar, el garante autoriza a Yego Rapidín a:</p>
              ...
              <p className="text-xs text-gray-600">Firma en el recuadro usando tu dedo o mouse.</p>
              <div className="border border-dashed border-gray-300 rounded-lg p-3 bg-gray-50">
                <canvas
                  ref={contactSignatureRef}
                  width={600}
                  height={200}
                  style={{ width: '100%', maxWidth: '100%', height: '180px', border: '1px solid #d1d5db', borderRadius: '0.5rem', backgroundColor: 'white', cursor: 'crosshair', touchAction: 'none' }}
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
          <div className="border-b border-gray-200 px-4 py-3">
            <h3 className="text-sm font-semibold text-gray-900">Datos de la Persona de Contacto</h3>
          </div>
          <div className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {contactPhoneError && (
                  <p className="text-xs text-red-600 mt-1">El teléfono del contacto no puede ser el mismo que el tuyo</p>
                )}
              </div>
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-600 focus:outline-none text-sm"
                >
...
function Step5PaymentOptions({ setFormData, loanOptions, loading, driverCycle, requestedAmount, country = 'PE' }: any) {
  ...
  if (!loanOptions?.option) {
    return (
      <div className="text-center py-12">
        <p className="text-sm text-gray-600">No hay plan disponible. Por favor, completa el paso anterior.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Tu Plan de Pago</h2>
          <p className="text-xs text-gray-600 mt-0.5">Según tu ciclo {cycle} – monto y cuotas correspondientes</p>
        </div>
      </div>
      ...
      <div className="bg-white border-2 border-red-600 rounded-lg overflow-hidden bg-red-50/30">
        <div className="p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div>
              ...
              <p className="text-base font-semibold text-gray-900">{formatCurrency(parseFloat(requestedAmount || '0') || 0, country)}</p>
            </div>
            <div>
              <p className="text-xs text-gray-600 mb-0.5">Cuota semanal:</p>
              <p className="text-base font-semibold text-gray-900">{formatCurrency(option.data?.weeklyInstallment ?? 0, country)}</p>
            </div>
            <div>
              <p className="text-xs text-gray-600 mb-0.5">Total a pagar:</p>
              <p className="text-base font-semibold text-gray-900">{formatCurrency(option.data?.totalAmount ?? 0, country)}</p>
            </div>
            <div>
              <p className="text-xs text-gray-600 mb-0.5">Tasa de interés:</p>
              <p className="text-base font-semibold text-gray-900">{option.data?.interestRate ?? '—'}% semanal</p>
            </div>
          </div>
          <button
            type="button"
        ...
        {expandedSchedule && schedule.length > 0 && (
                <div className="border-t border-gray-200 p-4 bg-gray-50 rounded-b-lg">
                  <h4 className="text-sm font-semibold text-gray-900 mb-3">Cronograma de Pagos</h4>
                  <div className="space-y-2.5 mb-4">
                    <div className="flex gap-3 p-3 rounded-lg bg-white border border-gray-200/80 shadow-sm">
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b border-gray-300">
                          <th className="text-left py-1.5 px-2 font-semibold text-gray-700">Cuota</th>
                          <th className="text-left py-1.5 px-2 font-semibold text-gray-700">Fecha de Vencimiento</th>
                          <th className="text-right py-1.5 px-2 font-semibold text-gray-700">Monto</th>
                        </tr>
                      </thead>
                      <tbody>
                        {schedule.map((item: any, idx: number) => (
                          <tr key={item?.installment ?? idx} className="border-b border-gray-200">
                            <td className="py-1.5 px-2">{item?.installment ?? idx + 1}</td>
                            ...
                            <td className="py-1.5 px-2 text-right font-semibold">{formatCurrency(item?.amount ?? 0, country)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <div>
          ...
          <p className="text-xs text-gray-600 mt-0.5">Lee y acepta los términos para continuar</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
        <div className="border-b border-gray-200 px-4 py-3">
          <h3 className="text-sm font-semibold text-gray-900">Condiciones del Préstamo</h3>
        </div>
        <div className="p-4 space-y-4">
          <div>
            <h4 className="text-xs font-semibold text-gray-900 mb-2">Proceso de solicitud:</h4>
          ...
          <div>
            <h4 className="text-xs font-semibold text-gray-900 mb-2">Condiciones de pago:</h4>
            <ul className="list-disc list-inside text-xs text-gray-700 space-y-1 leading-relaxed">
          ...
          <div>
            <h4 className="text-xs font-semibold text-gray-900 mb-2">Condiciones generales:</h4>
            <ul className="list-disc list-inside text-xs text-gray-700 space-y-1 leading-relaxed">
...
function Step7Signature({ formData, setFormData, signatureRef, clearSignature, handleSignatureStart, handleSignatureMove, handleSignatureEnd }: any) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Firma del Contrato y Foto de Documento de Identidad</h2>
          <p className="text-xs text-gray-600 mt-0.5">Firma el contrato y toma una foto de la parte frontal de tu documento de identidad</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
          <div className="border-b border-gray-200 px-4 py-3">
            <h3 className="text-sm font-semibold text-gray-900">1. Foto de Documento de Identidad (parte frontal)</h3>
          </div>
          <div className="p-4 space-y-3">
            ...
            {formData.idDocument ? (
              <div className="space-y-3">
                <div className="border border-gray-300 rounded-lg p-3 bg-gray-50">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs font-medium text-gray-900 truncate flex-1">{formData.idDocument.name}</p>
                    <button
                      type="button"
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
          <div className="border-b border-gray-200 px-4 py-3">
            <h3 className="text-sm font-semibold text-gray-900">2. Firma del Contrato</h3>
          </div>
          <div className="p-4 space-y-3">
            <p className="text-xs text-gray-600">
            ...
            <div className="border border-gray-300 rounded-lg overflow-hidden bg-white">
              <canvas
                ...
                width={600}
                height={200}
                style={{ width: '100%', maxWidth: '100%', height: '200px', cursor: 'crosshair', touchAction: 'none' }}
      ...
      {/* Modal cámara: solo cámara, sin opción de galería, con marco para encajar el DNI */}
      {showCamera && (
        <div className="fixed inset-0 z-50 flex flex-col bg-black">
          <video
            ref={videoRef}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 flex gap-3 justify-center bg-gradient-to-t from-black/80 to-transparent">
            <button
              type="button"
              onClick={closeCamera}
              className="flex items-center gap-2 px-5 py-2.5 bg-gray-600 text-white rounded-lg text-sm font-medium hover:bg-gray-500"
            >
              <X className="w-4 h-4" />
              Cancelar
            </button>
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700"
            >
              <Camera className="w-4 h-4" />
              Capturar
            </button>
