...
test('el detalle administrativo usa una ruta consolidada sin retirar las rutas existentes', () => {
  ...
  const solicitudService = read('yego_miauto/services/solicitud/miautoSolicitudService.js');
  ...
  assert.match(solicitudService, /requisitos_vehiculo/);
  assert.match(solicitudService, /requisitos_gastos/);
});
